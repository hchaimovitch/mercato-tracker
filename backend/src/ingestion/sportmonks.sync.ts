...
export async function synchroniserRumeurs(): Promise<void> {
  ...
  const debut = getEtat('sportmonks_derniere_sync') ?? new Date(Date.now() - 3 * 86_400_000).toISOString().slice(0, 10);
  ...
  for (const r of rumeurs) {
    ...
    const clubEntrant = r.toTeam ? trouverClubBig5ParNom(r.toTeam.name) : undefined;
    const clubSortant = r.fromTeam ? trouverClubBig5ParNom(r.fromTeam.name) : undefined;
    if (!clubEntrant && !clubSortant) continue; // ni club connu ni correspondance de nom — on ignore plutôt que de deviner

    const fenetre = fenetrePourDate(r.date);
    if (!fenetre) continue;
  ...
  setEtat('sportmonks_derniere_sync', fin);
